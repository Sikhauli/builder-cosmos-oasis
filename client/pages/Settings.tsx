...
const seedRoles: Role[] = [
  ...
  },
  {
    ...
    name: "Finance",
    mfa: true,
    permissions: ["payroll.read", "payroll.submit", "gl.export"],
  },
  {
    id: crypto.randomUUID(),
    name: "Auditor",
  ...
  return (
    <div className="container mx-auto py-4">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-xs text-muted-foreground">
            RBAC with predefined and custom roles, MFA per role, sessions,
            masking, residency controls, and exportable audit log.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        <section className="rounded-md border p-3 lg:col-span-2">
          <header className="mb-2 inline-flex items-center gap-2 text-xs font-medium">
            <UserCog className="h-3.5 w-3.5" /> Roles & permissions
          </header>
          <div className="mb-2 flex items-center gap-2 text-[11px]">
            ...
            />
            <Button className="h-7 px-2 text-xs" onClick={addRole}>
              Add role
            </Button>
          </div>
        ...
        <section className="rounded-md border p-3">
          <header className="mb-2 inline-flex items-center gap-2 text-xs font-medium">
            <Globe className="h-3.5 w-3.5" /> Data residency
          </header>
          <select
          ...
          <p className="mt-2 text-[10px] text-muted-foreground">
            Storage region used for documents and backups.
          </p>
        </section>
      </div>
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-3">
        <section className="rounded-md border p-3">
          <header className="mb-2 inline-flex items-center gap-2 text-xs font-medium">
            <ShieldCheck className="h-3.5 w-3.5" /> Masking
          </header>
          <div className="text-[11px]">
            SSN: {showSensitive ? "123-45-6789" : "***-**-****"}
          </div>
          <div className="text-[11px]">
            Bank: {showSensitive ? "1234 5678 9012" : "**** **** 9012"}
          </div>
          <Button
            variant="outline"
          ...
          <p className="mt-2 text-[10px] text-muted-foreground">
            Only privileged roles may reveal sensitive fields.
          </p>
        </section>

        <section className="rounded-md border p-3 lg:col-span-2">
          <header className="mb-2 inline-flex items-center gap-2 text-xs font-medium">
            <KeyRound className="h-3.5 w-3.5" /> Active sessions
          </header>
